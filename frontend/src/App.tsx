...
function App() {
  ...
  const [refreshToken, setRefreshToken] = useState<string | null>(() => window.localStorage.getItem("sv_refresh"));
  ...
  useEffect(() => {
    if (refreshToken) {
      window.localStorage.setItem("sv_refresh", refreshToken);
    } else {
      window.localStorage.removeItem("sv_refresh");
    }
  }, [refreshToken]);
  ...
  const handleLogin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const { access, refresh, user: u } = response.data as { access: string; refresh: string; user: { id: number; first_name: string; last_name: string; username: string; email: string } };
      setToken(access);
      setRefreshToken(refresh);
      setUser({ id: u.id, firstName: u.first_name, lastName: u.last_name, username: u.username, email: u.email });
      navigate("home");
